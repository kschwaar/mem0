...
<Tip>
  Python relationship graph support is default-off and currently a preview. See the <Link href="/open-source/features/relationship-graph-preview">relationship graph guide</Link> for Neo4j configuration, projection operations, reset behavior, and the retrieval quality gate.
</Tip>
