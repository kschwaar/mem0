...
description: "Guide for self-hosted Mem0 users to upgrade to the new memory algorithm with single-pass extraction, hybrid search, and entity-aware retrieval."
icon: "arrow-right"
iconType: "solid"
---
...
- **Extraction**: Single-pass action-aware extraction in Python (one LLM call chooses ADD/UPDATE/DELETE/NONE); TypeScript remains ADD-only
...
| `add()` events | Returns `ADD`, `UPDATE`, `DELETE` | Python returns `ADD`, `UPDATE`, or `DELETE`; TypeScript returns `ADD` only | Handle the event values supported by your SDK |
...
<Tip>
In the Python SDK, a new mutable fact can update an existing memory in place, so current preferences and status fields do not accumulate contradictory rows. Independent events and historical facts still accumulate. The TypeScript OSS SDK remains ADD-only.
</Tip>
...
### Extraction: Single-Pass Action-Aware (Python)

```
Input conversation
    → Retrieve top-10 related existing memories (for deduplication and conflict context)
    → Single LLM call: choose ADD, UPDATE, DELETE, or NONE per fact
    → Batch embed extracted memories
    → Hash-based deduplication (MD5, prevents exact duplicates)
    → Update/delete superseded mutable facts; batch-insert new facts
    → Entity extraction (for entity matching)
```

The previous algorithm used two LLM calls: one to extract candidate facts and one to decide actions. The Python pipeline now performs both tasks in one call. It updates mutable current-state facts in place, deletes explicit retractions, and preserves independent events as additions. The TypeScript OSS pipeline remains ADD-only.
